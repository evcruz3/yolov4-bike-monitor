        ...
        with slim.arg_scope([slim.conv2d, slim.batch_norm, _fixed_padding], reuse=reuse):
            with slim.arg_scope([slim.conv2d],
                ...
                with tf.variable_scope('yolo-v4-tiny'):
                    ...
                    #custom:
                    #net = _conv2d_fixed_padding(inputs,16,kernel_size=3,strides=1)
                    #net = net = slim.max_pool2d(net, [2, 2], scope='pool2')
                    #net = _conv2d_fixed_padding(net,32,kernel_size=3,strides=1)
                    #net = net = slim.max_pool2d(net, [2, 2], scope='pool2')
                    #net = _conv2d_fixed_padding(net,64,kernel_size=3,strides=1)
                    #net = net = slim.max_pool2d(net, [2, 2], scope='pool2')
                    #net = _conv2d_fixed_padding(net,128,kernel_size=3,strides=1)
                    #feat0 = net
                    #net = slim.max_pool2d(net, [2, 2], scope='pool2')
                    #net = _conv2d_fixed_padding(net,256,kernel_size=3,strides=1)
                    #feat = net
                    #net = net = slim.max_pool2d(net, [2, 2], scope='pool2')
                    #net = _conv2d_fixed_padding(net,512,kernel_size=3,strides=1)
                    #net = net = slim.max_pool2d(net, [2, 1], scope='pool2')
                    #net = _conv2d_fixed_padding(inputs,1024,kernel_size=3,strides=1)
                    
                    ############
                    #net = _conv2d_fixed_padding(net,256,kernel_size=1,strides=1)
                    #route = net
                    #net = _conv2d_fixed_padding(net,512,kernel_size=3,strides=1)
                    ...
                    #net = _conv2d_fixed_padding(route,128,kernel_size=1)
                    #upsample_size = feat0.get_shape().as_list()
                    #net = _upsample(net, upsample_size, data_format)
                    #net = tf.concat([net,feat0], axis=1 if data_format == 'NCHW' else 3)
