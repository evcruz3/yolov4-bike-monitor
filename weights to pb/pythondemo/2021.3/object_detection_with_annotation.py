...
def build_argparser():
    ...
    args.add_argument("-o", "--output_directory",
                      help="Required. Specify the target output path for annotation files and annotated images", required=True, type=str)
...
def main():
    ...
    while (cap.isOpened() \
        ...
        if next_frame_id_to_show in completed_request_results:
            ...
            curr_frame_id = next_frame_id_to_show
            ...
            if objects:
                with open(args.output_directory + "/" + str(curr_frame_id) + ".txt", "w") as annotation_file:
                    cv2.imwrite(args.output_directory + "/" + str(curr_frame_id) + ".jpg", frame)
                    for obj in objects:
                        ...
                        X_CENTER_NORM = ((obj['xmin'] + obj['xmax'])/2)/origin_im_size[1]
                        Y_CENTER_NORM = ((obj['ymin'] + obj['ymax'])/2)/origin_im_size[0]
                        WIDTH_NORM = (obj['xmax'] - obj['xmin'])/origin_im_size[1]
                        HEIGHT_NORM = (obj['ymax'] - obj['ymin'])/origin_im_size[0]
                                       
                        annotation_file.write("{:-1} {:-1} {:-1} {:-1} {:-1}\n".format(LABEL_ID, X_CENTER_NORM, Y_CENTER_NORM, WIDTH_NORM, HEIGHT_NORM))
